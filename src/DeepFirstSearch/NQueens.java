...
class NQueensSolution2 {
    ...
    public ArrayList<String[]> solveNQueens(int n) {
        ...
        dfs(result, locations, 0, n);
        return result;
    }

    public void dfs(ArrayList<String[]> result, int[] locations, int cur, int n) {
        if (cur == n) {
            printboard(result, locations, n);
        } else {
            for (int i = 0; i < n; i++) {
                locations[cur] = i;
                if (isValid(locations, cur)) {
                    dfs(result, locations, cur + 1, n);
                }
            }
        }
    }

    public boolean isValid(int[] location, int cur) {
        for (int i = 0; i < cur; i++) {
    ...
    public void printboard(ArrayList<String[]> result, int[] locations, int n) {
